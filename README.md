...
### Long-read / hybrid QC summary

```bash
pixi run genomepuzzle long qc \
  --sample-sheet hybrid_dataset/sample_sheet.csv \
  --dataset-dir hybrid_dataset \
  --output-csv hybrid_dataset/long_qc_summary.csv \
  --output-json hybrid_dataset/long_qc_summary.json
```

This produces per-sample QC summaries for the short-read pairs and long-read FASTQs, including simple flags for obviously low short-read or long-read counts relative to the dataset median.
...
- `genomepuzzle/long_qc.py`: long-read and hybrid dataset QC summaries
